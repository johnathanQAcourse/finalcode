...
class TestPlayer(TestCase):
    ...
    def test__init__1(self):
        ...
        self.assertEqual(player.players_deck, [])
    ...
    def test__init__4(self):
        """test to see if the cards_to_deal is default when you enter out of range cards_to_deal """
        player = Player("a", 27)
        self.assertTrue(player.name == "a")
        self.assertTrue(player.cards_to_deal == 26)
        self.assertTrue(len(player.players_deck) == 0)
    ...
    def test_set_hand1(self):
        """this is to test if the player's deck was dealt correctly, given a single card"""
        with patch("DeckOfCards.DeckOfCards.deal_one") as mock_deal_one:
            ...
            self.assertIn(mock_deal_one.return_value, self.player.players_deck)

    def test_set_hand2(self):
        """test of duplicates sent from deal_one not inserted to list"""
        with patch("DeckOfCards.DeckOfCards.deal_one") as mock_deal_one:
            mock_deal_one.return_value = Card(3, 2)
            deck = DeckOfCards()
    ...
    def test_set_hand_4_invalidtypeofDeck(self):
        """test when the deck variable is not of DeckOfCards type."""
        p1 = Player("a", 26)
        with self.assertRaises(TypeError):
            p1.set_hand("deck")
        with self.assertRaises(TypeError):
    ...
    def test_set_hand_5(self):
        """the deck is a string"""
        with self.assertRaises(TypeError):
